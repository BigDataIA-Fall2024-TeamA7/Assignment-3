...
from utils.api_requests import fetch_documents  # Function to fetch data from FastAPI
...
def render():
    ...
    # Fetch document data from FastAPI
    ...
    if selected_document:
        ...
        if st.button("Open Document", key="open_doc"):
            ...
            st.session_state.page = 'open_document'  # Page reference updated
    ...
    if st.button("Back to Landing Page", key="back_to_landing"):
        navigate_to('landing')  # Navigate back to the landing page
    ...
    st.markdown(
        ...
        div.stButton > button:first-child {
            background-color: #1E90FF; color: #FFF; border-radius: 8px; padding: 10px 20px; font-size: 18px; font-weight: bold; margin-top: 10px;
        }
        div.stButton > button:nth-child(2) {
            background-color: #FF6347; color: #FFF; border-radius: 8px; padding: 10px 20px; font-size: 18px; font-weight: bold; margin-top: 10px;
        }
        </style>
        """,
        unsafe_allow_html=True
