...
from utils.navigation import navigate_to  # Utility function for page navigation
    ...
    if document:
        ...
        with col1:
            ...
            if st.button("Summarize Document", key="summarize_button", help="Click to generate a summary"):
                st.session_state.show_summary_box = True  # Show the summary box when clicked

            # Display an empty text box for the summary with scrollbar, highlighted if button clicked
            if st.session_state.get("show_summary_box", False):
                st.markdown(
                    "<div style='margin-top: 10px; padding: 10px; border: 2px solid #FF6347; border-radius: 5px; background-color: #333;'>"
